...
# ──────────────────────────────────────────────────────────────
def score_group_a(
    avg_views, avg_like_ratio, avg_age_months, avg_comments, avg_vpm, S: dict
):
    """
    ...
    if avg_views < S["a_avg_views_lo"]:
        pts["Avg Views"] = round(P / 2)  # too few → unproven
    elif avg_views <= S["a_avg_views_hi1"]:
        pts["Avg Views"] = P  # ✓ sweet spot
    elif avg_views <= S["a_avg_views_hi2"]:
        pts["Avg Views"] = round(P / 2)  # competitive
    else:
        pts["Avg Views"] = 0  # dominated

    # Avg Video Age — sweet spot = fresh but proven (not brand-new, not ancient)
    P = S["a_video_age_pts"]
    if avg_age_months < S["a_video_age_lo_mo"]:
        pts["Avg Video Age"] = round(P / 2)  # too new
    elif avg_age_months <= S["a_video_age_hi1_mo"]:
        pts["Avg Video Age"] = P  # ✓ sweet spot
    elif avg_age_months <= S["a_video_age_hi2_mo"]:
        pts["Avg Video Age"] = round(P / 2)  # getting old
    else:
        pts["Avg Video Age"] = 0  # stale

    # Views per Month — sweet spot = active niche, not a viral giant
    P = S["a_vpm_pts"]
    if avg_vpm < S["a_vpm_lo"]:
        pts["Views per Month"] = 0  # dead niche
    elif avg_vpm <= S["a_vpm_hi1"]:
        pts["Views per Month"] = P  # ✓ sweet spot
    elif avg_vpm <= S["a_vpm_hi2"]:
        pts["Views per Month"] = round(P / 2)  # viral/competitive
    else:
        pts["Views per Month"] = 0  # dominated by viral content
    ...
    if ratio_pct < S["a_like_ratio_lo"]:
        pts["Likes / Views Ratio"] = 0  # not engaging
    elif ratio_pct <= S["a_like_ratio_hi1"]:
        pts["Likes / Views Ratio"] = P  # ✓ sweet spot
    elif ratio_pct <= S["a_like_ratio_hi2"]:
        pts["Likes / Views Ratio"] = round(P / 2)  # high (popular channels)
    else:
        pts["Likes / Views Ratio"] = 0  # dominated

    # Avg Comments — sweet spot = active discussion, not overwhelmingly competitive
    P = S["a_comments_pts"]
    if avg_comments < S["a_comments_lo"]:
        pts["Avg Comments"] = 0  # not engaging
    elif avg_comments <= S["a_comments_hi1"]:
        pts["Avg Comments"] = P  # ✓ sweet spot
    elif avg_comments <= S["a_comments_hi2"]:
        pts["Avg Comments"] = round(P / 2)  # heavy competition
    else:
        pts["Avg Comments"] = 0  # dominated
...
def score_group_c(kw_in_title: int, kw_in_desc: int, title_pos_score: int, S: dict):
    ...
    t2 = S["c_title_t2"]
    if kw_in_title == 0:
        pts["KW in Titles"] = P  # FULL points
    elif kw_in_title <= t1:
    ...
    d2 = S["c_desc_t2"]
    if kw_in_desc == 0:
        pts["KW in Descriptions"] = P  # FULL points
    elif kw_in_desc <= d1:
...
def get_video_details(yt, video_ids):
    resp = yt.videos().list(part="snippet,statistics", id=",".join(video_ids)).execute()
...
def analyse_keyword(yt, keyword: str, S: dict) -> dict:
    ...
    avg_age_months = (
        sum(age_months_list) / len(age_months_list) if age_months_list else 24
    )
...
def group_bar_html(score, max_score, color, title, breakdown: dict) -> str:
    ...
    rows = "".join(
        f"<tr><td>{m}</td><td class='pts'>{p}</td></tr>" for m, p in breakdown.items()
    )
...
def render_result(res: dict):
    ...
    with col_gauge:
        st.markdown(
            kos_gauge_html(
                res["kos"], res["kos_label"], res["kos_emoji"], res["kos_color"]
            ),
            unsafe_allow_html=True,
        )
    with col_a:
    ...
    # Top 4 channels
    st.markdown(
        '<div class="section-header">📺 Top 4 Channels by Subscribers</div>',
        unsafe_allow_html=True,
    )
    ch_df = pd.DataFrame(res["top4_channels"])[
        ...
        columns={
            "Subs (fmt)": "Subscribers",
            "Videos (fmt)": "Videos",
            "Total Views (fmt)": "Total Views",
            "Channel Age (yr)": "Age (yrs)",
        }
    )
    ch_df.index = list(range(1, len(ch_df) + 1))  # type: ignore[assignment]
    st.dataframe(
        ch_df,
    ...
    # Top 10 videos
    st.markdown(
        '<div class="section-header">🎬 Top 10 Videos</div>', unsafe_allow_html=True
    )
    vid_df = (
        pd.DataFrame(res["videos"])[
            [
                "Rank",
                "Title",
                "Channel",
    ...
    st.markdown(
        '<div class="section-header">🔤 Keyword Frequency per Video</div>',
        unsafe_allow_html=True,
    )
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        for r in results:
            if "error" in r:
                continue
            safe = (
                r["keyword"][:25].replace("/", "_").replace("\\", "_").replace("*", "_")
            )
            ...
            pd.DataFrame(bd_rows).to_excel(
                writer, sheet_name=f"{safe}_score", index=False
            )
...
with tab_single:
    ...
    with col_input:
        keyword = st.text_input(
            "Enter a keyword",
            placeholder="e.g. coffee and jazz",
...
""")
    ...
    if uploaded:
        try:
            ...
            keywords_list = df_kw.iloc[:, 0].dropna().astype(str).str.strip().tolist()
            keywords_list = [k for k in keywords_list if k]
            st.success(
                f"✅ Loaded **{len(keywords_list)} keywords** from `{uploaded.name}`"
            )
            st.dataframe(
                pd.DataFrame({"Keywords": keywords_list}),
                use_container_width=True,
                height=180,
            )
        except Exception as e:
        ...
        if run_bulk:
            ...
            for i, kw in enumerate(keywords_list):
                ...
                status_table.dataframe(
                    pd.DataFrame(status_rows), use_container_width=True
                )
...
with tab_settings:
    ...
    with st.expander("📹 Group A — Video Metrics", expanded=True):
        ...
        st.caption(
            "Each metric has a **lower bound** (unproven below it) and an **upper bound** (sweet spot end). Above the upper bound = too competitive."
        )
        ...
        with t1:
            ...
            S["a_avg_views_lo"] = st.number_input(
                "🔻 Too few (½ pts) — below",
                value=S["a_avg_views_lo"],
                step=10_000,
                key="sset_a_avg_views_lo",
                format="%d",
                help="Below this = not enough views to prove the niche.",
            )
            S["a_avg_views_hi1"] = st.number_input(
                "✅ Sweet spot upper — below",
                value=S["a_avg_views_hi1"],
                step=100_000,
                key="sset_a_avg_views_hi1",
                format="%d",
                help="Sweet spot: between lower bound and this = full pts.",
            )
            S["a_avg_views_hi2"] = st.number_input(
                "🔺 Dominated (0 pts) — above",
                value=S["a_avg_views_hi2"],
                step=1_000_000,
                key="sset_a_avg_views_hi2",
                format="%d",
                help="Above this = niche dominated by big channels.",
            )
            st.markdown("**Avg Comments**")
            S["a_comments_lo"] = st.number_input(
                "🔻 Not engaging (0 pts) — below",
                value=S["a_comments_lo"],
                step=5,
                key="sset_a_comments_lo",
                format="%d",
            )
            S["a_comments_hi1"] = st.number_input(
                "✅ Sweet spot upper — below",
                value=S["a_comments_hi1"],
                step=100,
                key="sset_a_comments_hi1",
                format="%d",
            )
            S["a_comments_hi2"] = st.number_input(
                "🔺 Dominated (0 pts) — above",
                value=S["a_comments_hi2"],
                step=500,
                key="sset_a_comments_hi2",
                format="%d",
            )
        with t2:
            ...
            S["a_like_ratio_lo"] = st.slider(
                ...
                0.0,
                2.0,
                float(S["a_like_ratio_lo"]),
                step=0.1,
                key="sset_a_like_ratio_lo",
            )
            S["a_like_ratio_hi1"] = st.slider(
                "✅ Sweet spot upper — below (%)",
                1.0,
                10.0,
                float(S["a_like_ratio_hi1"]),
                step=0.5,
                key="sset_a_like_ratio_hi1",
            )
            S["a_like_ratio_hi2"] = st.slider(
                "🔺 Dominated (0 pts) — above (%)",
                5.0,
                25.0,
                float(S["a_like_ratio_hi2"]),
                step=0.5,
                key="sset_a_like_ratio_hi2",
            )
        with t3:
            st.markdown("**Views per Month**")
            S["a_vpm_lo"] = st.number_input(
                "🔻 Dead niche (0 pts) — below",
                value=S["a_vpm_lo"],
                step=100,
                key="sset_a_vpm_lo",
                format="%d",
    ...
    with st.expander("📺 Group B — Channel Metrics", expanded=False):
        ...
        with gb1:
            S["b_subs_pts"] = st.slider(
                "Avg Subscribers (pts)", 0, 25, S["b_subs_pts"], key="sset_b_subs_pts"
            )
        with gb2:
        ...
        with tb2:
            ...
            S["b_vid_count_t1"] = st.number_input(
                "Very many (0 pts) >",
                value=S["b_vid_count_t1"],
                step=50,
                key="sset_b_vid_count_t1",
                format="%d",
            )
            S["b_vid_count_t2"] = st.number_input(
                "Many (½ pts) ≥",
                value=S["b_vid_count_t2"],
                step=10,
                key="sset_b_vid_count_t2",
                format="%d",
            )
        with tb3:
    ...
    with st.expander("📝 Group C — Content Saturation", expanded=False):
        ...
        with tc1:
            ...
            S["c_title_t1"] = st.slider(
                "Few (4/7 pts) — count ≤", 1, 10, S["c_title_t1"], key="sset_c_title_t1"
            )
            S["c_title_t2"] = st.slider(
                "Medium (2/7 pts) — count ≤",
                2,
                15,
                S["c_title_t2"],
                key="sset_c_title_t2",
        ...
        with tc2:
            ...
            S["c_desc_t1"] = st.slider(
                "Few (3/5 pts) — count ≤", 1, 10, S["c_desc_t1"], key="sset_c_desc_t1"
            )
            S["c_desc_t2"] = st.slider(
                "Medium (1/5 pts) — count ≤",
                2,
                15,
                S["c_desc_t2"],
                key="sset_c_desc_t2",
    ...
    if cur_max != 100:
        st.warning(
            f"Total is {cur_max}/100. Adjust point allocations to reach exactly 100."
        )
    ...
    with col_reset:
        if st.button("🔄 Reset All to Defaults", use_container_width=True):
            ...
            for k in list(st.session_state.keys()):
                if isinstance(k, str) and k.startswith("sset_"):
