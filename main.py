...
def analyse_keyword(api_key: str, keyword: str):
    ...
    print(f"\n{Fore.CYAN}{'─'*60}")
    print(f"  🔍  YouTube Keyword Evaluator")
    ...
    chan_map = {c["id"]: c for c in channel_details}
    ...
    channel_view_acc = defaultdict(int)   # accumulate views per channel
    ...
    for chan in channel_details:
        ...
        channel_rows.append((subs, name, fmt_number(subs), fmt_number(vids), fmt_number(ttl_v), url))
    ...
    print(f"  📺  TOP 4 CHANNELS  (by subscribers)")
    print(f"{'━'*60}{Style.RESET_ALL}")
    ...
    print(f"  🎬  TOP 10 VIDEOS")
    print(f"{'━'*60}{Style.RESET_ALL}")
    vid_headers = ["#", "Title", "Channel", "Views", "Likes", "Comments",
                   "KW in Title", "KW in Desc", "URL"]
